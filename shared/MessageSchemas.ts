...
const physicsSchema = BufferSchema.schema('physics', {
  // Velocity with 2 decimal precision
  velocityX: { type: int16, digits: 2 },
  velocityY: { type: int16, digits: 2 },
  velocityZ: { type: int16, digits: 2 },
  
  // Physics flags packed into a single byte
  flags: uint8 // bit 0: isStatic, bit 1: useGravity, bit 2: isGrounded, etc.
});

// Render component schema  
const renderSchema = BufferSchema.schema('render', {
  // Color as RGB packed into 3 bytes
  colorR: uint8,
  colorG: uint8,
  colorB: uint8,
  
  // Mesh scale with 2 decimal precision
  meshScale: { type: uint16, digits: 2 }
});

// Network component schema
const networkSchema = BufferSchema.schema('network', {
  authorityType: uint8, // 0: server, 1: client, 2: shared
  lastUpdateSequence: uint32
});

// Entity schema combining all components
const entitySchema = BufferSchema.schema('entity', {
  ...
  // Component flags to indicate which components are present
  componentFlags: uint8, // bit 0: transform, bit 1: physics, bit 2: render, bit 3: network
  
  // Components (when present based on flags)
...
// Create models for efficient serialization/deserialization with larger buffers
export const GameStateModel = new Model(gameStateSchema, 64); // Much larger buffer for many entities
